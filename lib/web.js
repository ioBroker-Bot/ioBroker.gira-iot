...
function GiraWebExtension(server, webSettings, adapter, instanceSettings, app) {
    ...
    this.unload = () => {
        return new Promise(resolve => {
            ...
            const middlewareIndex = app._router.stack.findIndex(
                layer => layer && layer.route === `/${this.namespace}/`,
            );
    ...
    (function __constructor() {
        if (!that.settings.secure) {
            that.adapter.log.error(
                `Unable to register ${that.namespace} web extension - HTTPS is disabled on web instance`,
            );
        } else if (that.adapter.config.bind === '0.0.0.0') {
            that.adapter.log.error(
                `Unable to register ${that.namespace} web extension - please select specific interface (other address than 0.0.0.0)`,
            );
        } else {
